...
const linkStyle = `
.link-field.form-field:not(.editing) {
  .field-label {
...
const linkField = {
  ...
  template: function (fieldData) {
    const { label = 'Link', style, href, target } = fieldData
    const classNames = ['link-control']
    ...
    a.appendChild(styleElement)

    if (target === '_blankWindow') {
      a.addEventListener('click', evt => {
        evt.preventDefault()
        const { windowSize } = fieldData
        ...
        const optionsStr = Object.entries(mergedOptions)
          .map(([key, value]) => `${key}=${value}`)
          .join(',')

        window.open(href, '_blank', optionsStr)
      })
    }
    ...
    return {
      ...
      layout: 'noLabel',
      onRender: evt => {
        // console.log('Link field rendered:', evt)
      }
    }
  }
}
